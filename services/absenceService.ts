...
export async function createAbsence(absence: { className: any; reason: any; type: any; studentId: any; }){
...
export async function deleteAbsence(id){
    const con = await getConnection();
    const [result] = await con.query<OkPacket>("delete from Absences where Id = ?",[id]);
    return result;
}